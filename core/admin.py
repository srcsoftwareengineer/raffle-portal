...
class RaffleDrawLog(models.Model):
    raffle = models.ForeignKey(
        "Raffle", on_delete=models.CASCADE, related_name="draw_logs"
    )
    ticket = models.ForeignKey(
        "Ticket", on_delete=models.CASCADE, related_name="draw_logs"
    )
    drawn_at = models.DateTimeField(auto_now_add=True)
    drawn_by = models.ForeignKey(User, null=True, blank=True, on_delete=models.SET_NULL)
    is_redraw = models.BooleanField(default=False)

    def __str__(self):
        status = "Re-sorteio" if self.is_redraw else "Sorteio"
        return f"{status} da rifa #{self.raffle.id} - Ticket #{self.ticket.number} em {self.drawn_at}"
